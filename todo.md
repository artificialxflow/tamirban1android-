...
- [x] **صفحه جزئیات ویزیت**  
  - [x] طراحی صفحه جزئیات ویزیت با نمایش تمام اطلاعات (وضعیت، مشتری، بازاریاب، تاریخ/زمان، یادداشت، اقدام پیگیری، موقعیت)
  - [x] دکمه ویرایش ویزیت در صفحه جزئیات
  - [x] دکمه تغییر وضعیت ویزیت  

### 8. فاز ماژول Visits
- [x] **لیست ویزیت‌ها و داشبورد ساده**  
  - [x] پیاده‌سازی صفحه Visits مشابه `handoff/static-ui/dashboard.html` و `components/visits/visits-page-client.tsx`  
  - [x] اتصال به `GET /api/visits` با فیلترهای تاریخ، بازاریاب، وضعیت، مشتری  
  - [x] نمایش کارت‌های ویزیت با وضعیت، یادداشت‌ها و اطلاعات مشتری  
- [x] **فرم ثبت/ویرایش ویزیت**  
  - [x] فرم ایجاد ویزیت جدید (انتخاب مشتری، بازاریاب، تاریخ شمسی، ساعت، یادداشت‌ها و اقدام پیگیری)  
  - [x] استفاده از Persian DateTime Picker با پکیج `shamsi_date`  
  - [x] اتصال به `POST /api/visits`, `PATCH /api/visits/{id}`, `DELETE /api/visits/{id}`
  - [x] دکمه‌های ویرایش و حذف در کارت ویزیت
- [x] **صفحه جزئیات ویزیت**  
  - [x] طراحی صفحه جزئیات ویزیت با نمایش تمام اطلاعات
  - [x] دکمه ویرایش و تغییر وضعیت در صفحه جزئیات
  - [x] اتصال به `PATCH /api/visits/{id}/status` برای تغییر وضعیت  
...
- [x] **لیست پیش‌فاکتورها**  
  - [x] طراحی UI مطابق `handoff/static-ui/invoices.html` و `components/invoices/*`  
  - [x] اتصال به `GET /api/invoices` با فیلتر وضعیت و تاریخ + pagination  
  - [x] نمایش کارت‌های پیش‌فاکتور با وضعیت، مبلغ و تاریخ
- [x] **ایجاد/ویرایش پیش‌فاکتور**  
  - [x] فرم انتخاب مشتری، آیتم‌ها (عنوان، تعداد، قیمت، تخفیف، مالیات)، تاریخ صدور و سررسید (تاریخ شمسی)  
  - [x] محاسبه خودکار جمع کل و نمایش خلاصه (Subtotal, Discount, Tax, Grand Total)  
  - [x] اتصال به `POST /api/invoices`, `PATCH /api/invoices/{id}`, `DELETE /api/invoices/{id}`  
- [x] **جزئیات پیش‌فاکتور و وضعیت پرداخت**  
  - [x] صفحه جزئیات Invoice با اطلاعات کامل مشتری و آیتم‌ها  
  - [x] اتصال به `PATCH /api/invoices/{id}/status` برای تغییر وضعیت (DRAFT, SENT, PAID, OVERDUE, CANCELLED)  
  - [x] نمایش تاریخ و مرجع پرداخت در صورت PAID  
  - [x] دکمه‌های ویرایش و تغییر وضعیت در صفحه جزئیات
...
- [x] **لیست بازاریاب‌ها و کارت‌های عملکرد**  
  - [x] طراحی صفحه مطابق `components/marketers/*` و بخش مربوط در Dashboard وب  
  - [x] اتصال به `GET /api/marketers` با فیلتر و Pagination  
  - [x] نمایش کارت‌های بازاریاب با اطلاعات کامل
- [x] **ایجاد/ویرایش/حذف بازاریاب**  
  - [x] فرم ایجاد/ویرایش بازاریاب (نام، شماره، ایمیل، نقش، منطقه، وضعیت فعال/غیرفعال)  
  - [x] اتصال به `POST /api/marketers`, `PATCH /api/marketers/{id}`, `DELETE /api/marketers/{id}`  
  - [x] دکمه‌های ویرایش و حذف در کارت بازاریاب
...
- **رفع مشکل احراز هویت:** اصلاح ساختار پارس کردن پاسخ `verifyOtp` - توکن‌ها اکنون به درستی از `data['accessToken']` و `data['refreshToken']` خوانده می‌شوند (نه از `data['tokens']`)
- **تنظیم آیکن اپلیکیشن:** استفاده از `sample/public/favicon.png` به عنوان آیکن اپلیکیشن برای Android و iOS با `flutter_launcher_icons`
